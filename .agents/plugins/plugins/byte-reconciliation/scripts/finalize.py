"""Build the final reconciliation result and optional Paseo memory payload."""
...
def run_node(input: dict[str, Any]) -> dict[str, Any]:
    ...
    stage_fields = [
        ("analysis_summary", "对账需求与口径", "project", ("reconciliation", "requirements")),
        ("design_summary", "对账规则设计", "decision", ("reconciliation", "design")),
        ("repository_summary", "对账实现", "project", ("reconciliation", "implementation")),
        ("local_test_summary", "对账本地测试", "procedure", ("reconciliation", "testing")),
        ("review_summary", "对账测试与比对结论", "decision", ("reconciliation", "comparison")),
    ]
    ...
    return {
        "data": {
            ...
            "rule_id": _text(origin.get("rule_id"), 256),
            "stages": {
