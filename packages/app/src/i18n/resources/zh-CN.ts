...
export const zhCN: TranslationResources = {
  common: {
    ...
    errors: {
      ...
      daemonClientDisconnected: "Daemon 客户端已断开连接",
  ...
  workflows: {
    ...
    guide: {
      ...
      nodes: {
        ...
        for: "• For：支持数组、数字倒计时和持续循环。每轮都从 For 节点的 input.data 开始，跨轮状态请使用 loop 变量。",
      },
    },
    host: {
      select: "选择主机",
    ...
    inputContract: {
      ...
      outputSchema: "Workflow output.data 结构约束",
      outputSchemaHint: "Workflow 成功结束前校验最终 output.data",
      nodeVariables: "节点变量",
      nodeVariablesHint: '声明仅当前节点可读的常量，不能修改；类型支持 "string" 和 "int64"。',
    ...
    editor: {
      untitled: "未命名工作流",
      unsaved: "未保存",
      generatedFileName: "首次保存后自动生成文件名",
      name: "工作流名称",
      nodes: "个节点",
      description: "描述",
      ...
      testRunUpstreamHint:
        "使用前一节点的 output；如果没有前一节点，则使用 Workflow 原始输入测试所选节点。",
      testRunNodeInputHint: "将完整的本节点 input 直接传给所选节点，不执行框架字段映射或变量填充。",
      inputJson: "输入 JSON",
      nodeInputJson: "本节点输入 JSON",
      ...
      inputMode: "输入类型",
      inputModeHint: "指定上方 JSON 如何转换为所选节点的 input。",
      selectInputMode: "选择输入类型",
      inputModes: {
    ...
    nodes: {
      ...
      contract: {
        title: "节点数据契约",
        description:
          "生成 input.data、声明节点变量，并分别校验节点接收的 input.data 和生成的 output.data。",
        inputs: "input.data 映射（可选）",
        inputsHint:
          "在节点执行前生成本节点的 input.data。留空时沿用上一个节点的 output.data；可用于筛选、重命名或组合字段。例如：{{nodes.scan.outputs.items}}。",
        inputSchema: "input.data 结构校验",
        outputSchema: "output.data 结构校验（可选）",
        schemaHint: "可选。Paseo 会在继续执行前校验对象。",
        invalidJson: "请输入合法 JSON。",
        objectRequired: "必须填写 JSON 对象。",
      },
      contractHints: {
        inputSchema: "校验完成映射后，本节点实际接收的 input.data",
        outputSchema:
          "校验本节点生成的 output.data，用于及时定位输出格式错误；不替代下游节点的 input.data 结构校验。",
      },
      schemaCompatibility: {
        title: "data 结构兼容性提示",
        workflowOutput: "Workflow 的 output.data",
        requiredNotGuaranteed:
          "{{source}} → {{target}}：output.data Schema 未保证必填字段“{{path}}”",
        fieldNotAccepted:
          "{{source}} → {{target}}：output.data 可能包含字段“{{path}}”，但下游 input.data Schema 不接受该字段",
        typeMismatch:
          "{{source}} → {{target}}：output.data 字段“{{path}}”类型为 {{outputTypes}}，下游 input.data Schema 要求 {{inputTypes}}",
      },
      help: {
      ...
      agent: {
        ...
        lifecycleHint:
          "决定同一个 Agent 节点创建的 Agent 可以复用多久；复用 Agent 的调用会按顺序执行。",
        selectLifecycle: "选择 Agent 生命周期",
        lifecycleOptions: {
          workflow: "整个 Workflow",
          workflowDescription: "首次执行时创建，之后复用，Workflow 结束时释放。",
          for: "本 For 循环",
          forDescription: "首次迭代时创建，在当前 For 循环内复用，跳出循环时释放。",
          single: "单次",
          singleDescription: "每次执行都创建新的 Agent，不复用。",
        },
        subsequentPromptMode: "非首次提示词",
        subsequentPromptModeHint: "设置本节点再次调用已初始化 Agent 时发送的提示词。",
        selectSubsequentPromptMode: "选择非首次提示词方式",
        subsequentPromptModes: {
          reuseInitial: "复用初始提示词",
          reuseInitialDescription: "继续使用初始提示词模板，并按本次节点输入重新渲染变量。",
          custom: "自定义",
        ...
        archive: "完成后归档 Agent",
        archiveAtLifecycleEnd: "Agent 生命周期结束后自动归档",
        network: "允许访问网络",
        webSearch: "允许联网搜索",
      ...
      for: {
        ...
        selectMode: "选择循环类型",
        executionMode: "执行模式",
        executionModeHint: "串行按顺序执行每轮；并行同时执行多轮。",
        executionSerial: "串行",
        executionParallel: "并行",
        selectExecutionMode: "选择执行模式",
        items: "循环项表达式",
        itemsHint: "数组模式必须解析为 JSON 数组；数字模式必须解析为非负整数。",
        maximumIterations: "最大循环次数",
        maximumIterationsHint: "最多执行的循环次数，默认为 100；0 表示不限制。",
        concurrency: "并发度",
        concurrencyHint:
          "并行模式下同时执行的循环次数，默认为 1，最大为 100。每轮都从 For 节点的同一份 input.data 开始。",
        control: "循环控制表达式",
        controlHint:
  ...
  composer: {
    ...
    input: {
      ...
      terminalAccessibilityLabel: "终端提示符",
    ...
    errors: {
      ...
      fileTooLarge: "{{fileName}} 过大（最大 {{size}}）",
    },
    clientCommands: {
  ...
  workspace: {
    ...
    setup: {
      ...
      waiting: "正在初始化 Workspace...",
      empty: {
        noCommands: "此 Workspace 没有运行初始化命令。",
      },
      accessibility: {
        noCommands: "此 Workspace 没有运行初始化命令",
        log: "Workspace 初始化日志",
    ...
    git: {
      actions: {
        ...
        mergePr: {
          ...
          pending: "正在合并 PR...",
          success: "PR 已 merge",
          squash_mr: "合并 MR（squash）",
          merge_mr: "合并 MR（merge）",
          rebase_mr: "合并 MR（rebase）",
          pending_mr: "正在合并 MR...",
        ...
        unavailable: {
          ...
          mergeNoBase: "无法合并，因为无法确定基准分支",
          mergeDirty: "有本地变更时无法 merge，请先 commit 或 stash",
          mergeNothing: "无法 merge，因为此分支没有可 merge 的新内容",
          updateNoBase: "无法更新，因为无法确定基准分支",
          updateDirty: "有本地变更时无法更新，请先 commit 或 stash",
          updateCurrent: "无法更新，因为此分支已与 {{baseRef}} 保持最新",
          mergePrNoGithub: "当前无法合并 PR，因为 GitHub 未连接",
          archiveNotWorktree: "此处无法归档，因为此 Workspace 不是作为 Paseo Worktree 创建的",
          mergePrNoForge: "当前无法合并 {{noun}}，因为 {{brand}} 未连接",
          mergePrMissing: "无法合并 PR，因为还没有 PR",
          mergePrDraft: "无法合并 PR，因为 PR 仍是草稿",
          mergePrMerged: "无法合并 PR，因为 PR 已合并",
          mergePrClosed: "无法合并 PR，因为 PR 已关闭",
          mergePrConflicts: "无法合并 PR，因为 PR 存在冲突",
          mergePrQueue: "此处无法合并 PR，因为此仓库使用合并队列",
          mergePrNotReady: "{{brand}} 报告 {{noun}} 可合并后才能合并 {{noun}}",
      ...
      pr: {
        ...
        errors: {
          statusLoadFailed: "无法加载 PR 状态",
          activityLoadFailed: "无法加载 PR 动态",
        },
      },
      forgeSetup: {
  ...
  newWorkspace: {
    ...
    launch: {
      ...
      commandPlaceholder: "输入要运行的命令，留空则启动空白终端",
    },
  },
  desktop: {
    windowName: {
    ...
    permissions: {
      ...
      microphone: {
        ...
        requestsWebOnly: "桌面麦克风请求仅在 Web 运行环境中可用。",
      ...
      testNotification: {
        ...
        notDelivered: "通知未送达。请检查“系统设置 > 通知”。",
        failed: "发送通知失败。",
      },
    },
    integrations: {
  ...
  workspaceSetup: {
    ...
    errors: {
      ...
      selectModel: "请选择模型",
      hostDisconnected: "Host 未连接",
      pendingRequired: "没有待处理的 Workspace 初始化",
      composerStateRequired: "Workspace 初始化输入状态必填",
    },
  },
  onboarding: {
    title: "欢迎使用 Paseo",
  ...
  pairing: {
    ...
    direct: {
      ...
      errors: {
        ...
        tlsError: "TLS 错误。只有 Daemon 前方配置了 TLS 终止代理时，直连才使用 SSL。",
    ...
    scan: {
      ...
      webUnavailableBody: "Web 版本不支持二维码扫描。请改用“粘贴链接”。",
      backToSettings: "返回设置",
  ...
  settings: {
    ...
    diagnostics: {
      ...
      app: {
        title: "应用诊断",
        rowTitle: "应用诊断",
        rowHint: "收集连接、Daemon、Provider、桌面端和日志详情",
        run: "Run",
        running: "正在运行诊断...",
    ...
    host: {
      ...
      relay: {
        ...
        lan: {
          ...
          webApp: {
            title: "托管 Paseo 配对前端",
            hint: "通过此 Relay 的 HTTP 地址提供可解析的配对信息，并连接 Paseo Web 页面。",
            pathLabel: "配对前端路径",
      ...
      terminalProfiles: {
        ...
        moveDown: "下移",
        save: "Save",
        emptyState: "暂无终端配置。添加配置后可用指定命令启动终端。",
      },
      agentProfiles: {
      ...
      daemon: {
        ...
        update: {
          ...
          offlineTitle: "主机离线",
          offlineMessage: "该主机当前离线，请等待主机恢复在线后再更新。",
          requestFailedTitle: "更新失败",
          requestFailedMessage: "更新 Daemon 失败：{{error}}",
          dialogFailedMessage: "无法打开更新确认弹窗。",
        },
        dangerZone: "危险区域",
        remove: {
          title: "移除 Host",
    ...
    providers: {
      ...
      remove: {
        ...
        confirmMessage: "这会从 config.json 删除该 Provider 配置，且无法撤销。",
        confirm: "Remove",
        errorTitle: "无法移除 Provider",
      },
      statuses: {
        disabled: "已禁用",
        loading: "正在加载",
    ...
    project: {
      ...
      resources: {
        ...
        project: {
          title: "项目目录",
          hint: "单目录模式必须选择一个路径，并自动加入该目录下的 AI 通用知识目录；多目录模式可为空或配置多个路径，不会自动加入通用知识目录。",
          single: "单目录",
          multiple: "多目录",
          validation: {
      ...
      metadata: {
        ...
        commitMessage: "提交消息",
        commitMessagePlaceholder: "使用带作用域的 Conventional Commits",
        pullRequest: "PR",
        pullRequestPlaceholder: "先写摘要，并包含测试计划部分",
      },
      writeFailures: {
