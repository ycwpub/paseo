...
- **`feature_paseo` branch only — default UI copy is Simplified Chinese.** Apply this rule only when `git branch --show-current` returns `feature_paseo`; ignore it on every other branch. New user-visible titles, buttons, placeholders, empty states, errors, accessibility labels, plugin documents, and workflow UI copy must default to Simplified Chinese. Prefer i18n keys with a complete `zh-CN` translation. Keep protocol names, commands, paths, code, provider/model names, and established product terms such as Agent, Project, Workflow, MCP, PR, and ID unchanged when translation would reduce precision. Do not introduce an English-only fallback into the default interface. When touching an existing surface, translate nearby visible English copy in the same flow.
