...
export function readProjectConfigForProject(input: {
  ...
  const globalResult = readPaseoConfigFileForEdit(globalPath);
...
export function writeProjectConfigForProject(input: {
  ...
  const projectRoot =
    mode === "single"
      ? resolveSingleProjectRoot(input.project, parsed.data)
  ...
  const targetPath =
    mode === "multiple"
      ? resolveGlobalProjectConfigPath(input.paseoHome, input.project.projectId)
      : resolvePaseoConfigPath(projectRoot!);
  ...
  if (targetPath !== current.configPath && existsSync(current.configPath)) {
    ...
    try {
      ...
      removeEmptyGlobalConfigDirectory(input.paseoHome, current.configPath);
    } catch {
      return { ok: false, error: { code: "write_failed" } };
    }
  }
...
function resolveSingleProjectRoot(
  project: ProjectConfigStorageProject,
  config: PaseoConfigRaw,
): string | null {
  if (
...
function removeEmptyGlobalConfigDirectory(paseoHome: string, configPath: string): void {
  const globalRoot = path.resolve(paseoHome, GLOBAL_PROJECT_CONFIG_DIRECTORY);
  const parent = path.dirname(configPath);
  if (path.dirname(parent) !== globalRoot) return;
  try {
