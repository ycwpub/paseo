// Command permission demonstrates interactive approval of agent tool requests.
package main
...
func main() {
	...
	client, err := paseo.Dial(ctx, paseo.Options{
		...
		ClientID: "go-permission-example",
	...
	prompt := envOr(
		"PASEO_PROMPT",
		"创建一个名为 paseo-sdk-permission-demo.txt 的文件并写入 hello。",
	)
	if err := client.SendAgentMessage(ctx, agent.ID, prompt); err != nil {
		log.Fatal(err)
	}
	...
	for {
		...
		case event, ok := <-subscription.Events:
			...
			case "permission_requested":
				...
				response := paseo.DenyPermission("denied by the Go client user")
				if strings.EqualFold(strings.TrimSpace(answer), "y") {
					response = paseo.AllowPermission()
					// If the provider supplies explicit actions, preserve the
					// selected action identifier when approving.
					for _, action := range request.Actions {
						if action.Behavior == "allow" {
							response.SelectedActionID = action.ID
							break
						}
					}
				}
...
func resolveAgent(ctx context.Context, client *paseo.Client) (*paseo.Agent, error) {
	...
	sessionKey := envOr("PASEO_SESSION_KEY", "go-permission-example")
	agentTitle := "go-permission:" + sessionKey
	labels := map[string]string{"externalSessionId": sessionKey, "example": "permission"}
